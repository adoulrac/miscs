...
Sonata 2 – Fraud Detection Ecosystem

1. Data Centralization (Global Markets Data Warehouse)
	•	Database: Oracle Exadata (central repository for pre-trade data).
	•	Data Sources:
	•	Pre-trade data: Directly sourced from GM Data Warehouse.
	•	Post-trade data: BookArt database (used mainly for fixed income OTC models).
	•	Static data: Loaded from referential systems (RefRef → RefB2/Static Service, managed by QuantResearch Isto team).
	•	External sources: Bloomberg, Refinitiv.
	•	Internal sources: MDD (Market Data Database).
	•	Market data:
	•	Intraday data: Tick-by-Tick Service (Refinitiv).
	•	End-of-day data: Time Series Service (Bloomberg).
	•	Scope-based data ingestion:
	•	Data is loaded daily based on model-defined scopes (market abuse data marts).
	•	Static data is loaded globally, while market data is scope-specific.

2. Data Mart Creation
	•	Purpose:
	•	Store pre-processed SQL query results for fraud detection models.
	•	Apply functional and technical filters for controlled and justified data selection.
	•	Enhance performance and reduce CPU consumption on the database.
	•	Control Mechanism:
	•	Data Mart Traceability: Ensures all filtered messages are tracked and justified.
	•	Prevents data gaps and ensures complete visibility into filtered transactions.

3. Sonata 2 Fraud Detection Engine
	•	Deployment:
	•	Python application running on a Linux cluster (IV2, internal DMZ cloud).
	•	Automated deployment via Ansible scripts.
	•	Execution Workflow:
	1.	Connects to the Global Markets Data Warehouse.
	2.	Retrieves relevant data (static and dynamic queries).
	3.	Runs fraud detection models (scope-based execution).
	4.	Logs all filtered messages via Transaction Completeness Control.
	5.	Distributes processing across multiple Linux machines using multiprocessing.
	6.	Synchronizes with the database to reconcile processed data and alerts.

4. Alert Management & Distribution
	•	Risk Case Manager (RCM – ActOne):
	•	Central platform for compliance and Front Office Conduct Officers (FOCS).
	•	Receives alerts generated by Sonata 2 for investigation.
	•	API Alert Sender Module:
	•	Extracts alerts from the database and sends them to ActOne.
	•	Uses XML-based message format for integration.

5. Investigation & Reporting (Gravity Portal)
	•	Purpose:
	•	Front-end for exploring data stored in the Global Markets Data Warehouse.
	•	Provides additional context and raw data for alert investigations.
	•	Key Features:
	•	Raw data reports: Help compliance officers access complete transaction history.
	•	Market abuse data mart reports: Provide pre-processed insights for investigations.
	•	Overcomes RCM limitations (e.g., message display constraints).

This breakdown provides a clear overview of how the entire fraud detection ecosystem operates. Let me know if you need refinements!
