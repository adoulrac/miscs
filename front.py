...
“”“POST vers le backend FastAPI, retourne le JSON ou None.”””
try:
...
add_line(“error”, f”[API] Erreur HTTP {e.response.status_code}: {e.response.text[:120]}”)
return None

def api_stream(path: str, payload: dict):
“””
POST streaming vers /exec/stream.
Retourne un générateur de lignes (stdout: / stderr: / **END**).
“””
try:
with requests.post(
f”{API_BASE}{path}”, json=payload, stream=True, timeout=60
) as resp:
...
if run and cmd.strip():
    ...
    if is_streaming_command(cmd):
        # ── Mode streaming ──────────────────────────────────────────────
        add_line("system", f"[streaming] {cmd}")
        payload = {"namespace": ns, "pod": pod, "container": ctr, "command": cmd}
        for chunk in api_stream("/exec/stream", payload):
            if chunk == "__END__":
                break
            if chunk.startswith("stderr:"):
                ...
                add_line("output", chunk[7:] if chunk.startswith("stdout:") else chunk)
    else:
        # ── Mode one-shot ───────────────────────────────────────────────
        payload = {"namespace": ns, "pod": pod, "container": ctr, "command": cmd}
        result = api_post("/exec", payload)
        if result:
